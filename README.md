...
[![release](https://img.shields.io/badge/release-v0.3.0-blue)](https://github.com/cshaizhihao/OU-SSH)
...
交互式安装会询问是否已有解析到本机的域名；选择域名模式后使用 Caddy 自动申请 HTTPS 证书并自动续签。安装脚本也会询问是否将 SSH 登录端口修改为 `5522`，请保持当前 SSH 连接不要关闭，完成后先新开窗口测试新端口。
...
OU_SSH_CHANGE_SSH_PORT=1 \
OU_SSH_SSH_PORT=5522 \
bash <(curl -sL https://raw.githubusercontent.com/cshaizhihao/OU-SSH/main/install.sh)
```
